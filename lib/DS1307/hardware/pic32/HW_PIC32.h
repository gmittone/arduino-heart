...
void DS1307::begin()
{
	if ((_sda_pin == SDA) and (_scl_pin == SCL))
	{
		...
		I2C1ADD = DS1307_ADDR;									// Set I2C device address
...
void DS1307::_burstRead()
{
	if (_use_hw)
	{
		...
		I2C1TRN = (DS1307_ADDR<<1);							// Send device Write address
		while (I2C1STAT & (1 << _I2CSTAT_IWCOL))			// Check if there is a Write collision
		{
			...
			I2C1TRN = (DS1307_ADDR<<1);						// Retry send device Write address
		}
		...
		I2C1TRN = (DS1307_ADDR<<1) | 1;						// Send device Read address
		while (I2C1STAT & (1 << _I2CSTAT_IWCOL))			// Check if there is a Write collision
		{
			I2C1STATCLR = (1 << _I2CSTAT_IWCOL);			// Clear Write collision flag
			I2C1TRN = (DS1307_ADDR<<1) | 1;					// Retry send device Read address
		}
		while (I2C1STAT & (1 << _I2CSTAT_TRSTAT)) {}		// Wait for transmit to finish
		while (I2C1STAT & (1 << _I2CSTAT_ACKSTAT)) {}		// Wait for ACK
...
void DS1307::writeBuffer(DS1307_RAM r)
{
	if (_use_hw)
	{
		...
		while (I2C1STAT & (1 << _I2CSTAT_TRSTAT)) {}		// Wait for transmit to finish
		while (I2C1STAT & (1 << _I2CSTAT_ACKSTAT)) {}		// Wait for ACK
		I2C1TRN = 8;										// Send the 1st data byte
		while (I2C1STAT & (1 << _I2CSTAT_TRSTAT)) {}		// Wait for transmit to finish
		while (I2C1STAT & (1 << _I2CSTAT_ACKSTAT)) {}		// Wait for ACK
		for (int i=0; i<56; i++)
		{
...
DS1307_RAM DS1307::readBuffer()
{
	...
	if (_use_hw)
	{
		_waitForIdleBus();									// Wait for I2C bus to be Idle before starting
		I2C1CONSET = (1 << _I2CCON_SEN);					// Send start condition
		if (I2C1STAT & (1 << _I2CSTAT_BCL)) { return r; }	// Check if there is a bus collision
		while (I2C1CON & (1 << _I2CCON_SEN)) {}				// Wait for start condition to finish
		I2C1TRN = (DS1307_ADDR<<1);							// Send device Write address
		while (I2C1STAT & (1 << _I2CSTAT_IWCOL))			// Check if there is a Write collision
		{
		...
		while (I2C1STAT & (1 << _I2CSTAT_TRSTAT)) {}		// Wait for transmit to finish
		while (I2C1STAT & (1 << _I2CSTAT_ACKSTAT)) {}		// Wait for ACK
		I2C1TRN = 8;										// Send the register address
		while (I2C1STAT & (1 << _I2CSTAT_TRSTAT)) {}		// Wait for transmit to finish
		while (I2C1STAT & (1 << _I2CSTAT_ACKSTAT)) {}		// Wait for ACK
		_waitForIdleBus();									// Wait for I2C bus to be Idle before starting
		I2C1CONSET = (1 << _I2CCON_RSEN);					// Send start condition
...
void DS1307::poke(uint8_t addr, uint8_t value)
{
	if ((addr >=0) && (addr<=55))
	{
		...
		if (_use_hw)
		{
			...
			while (I2C1STAT & (1 << _I2CSTAT_TRSTAT)) {}		// Wait for transmit to finish
			while (I2C1STAT & (1 << _I2CSTAT_ACKSTAT)) {}		// Wait for ACK
			I2C1TRN = addr;										// Send the 1st data byte
			while (I2C1STAT & (1 << _I2CSTAT_TRSTAT)) {}		// Wait for transmit to finish
			while (I2C1STAT & (1 << _I2CSTAT_ACKSTAT)) {}		// Wait for ACK
			I2C1TRN = value;									// Send the 2nd data byte
			while (I2C1STAT & (1 << _I2CSTAT_TRSTAT)) {}		// Wait for transmit to finish
...
uint8_t DS1307::peek(uint8_t addr)
{
	if ((addr >=0) && (addr<=55))
	{
		...
		if (_use_hw)
		{
			...
			while (I2C1STAT & (1 << _I2CSTAT_TRSTAT)) {}		// Wait for transmit to finish
			while (I2C1STAT & (1 << _I2CSTAT_ACKSTAT)) {}		// Wait for ACK
			I2C1TRN = addr;										// Send the register address
			while (I2C1STAT & (1 << _I2CSTAT_TRSTAT)) {}		// Wait for transmit to finish
			while (I2C1STAT & (1 << _I2CSTAT_ACKSTAT)) {}		// Wait for ACK
			_waitForIdleBus();									// Wait for I2C bus to be Idle before starting
			I2C1CONSET = (1 << _I2CCON_RSEN);					// Send start condition
